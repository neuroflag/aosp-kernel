...
#define MACS "%02x%02x%02x%02x%02x%02x"
	 ...
	 *   2) wl_resume=2=0
	 *   3) insuspend=0x7
	 * Case 2: no pkt and event in suspend
	 *   1) dhd_master_mode=1
...
void
dhd_conf_check_connection(dhd_pub_t *dhd, int suspend)
{
	struct dhd_conf *conf = dhd->conf;
...
#endif
		dhd_conf_check_connection(dhd, suspend);
#ifdef SUSPEND_EVENT
