...
static int fts_read_touchdata(struct fts_ts_data *data)
{
    ...
    ret = fts_read_data(data, buf, 1, buf + 1, data->pnt_buf_size - 1);
    if (ret < 0) {
...
static void fts_irq_read_report(struct fts_ts_data *data)
{
    int ret = 0;
...
__attribute__((unused)) static int fts_ts_suspend(struct device *dev)
{
    int ret = 0;
    struct fts_ts_data *ts_data = fts_data;
...
__attribute__((unused)) static int fts_ts_resume(struct device *dev)
{
    struct fts_ts_data *ts_data = fts_data;
