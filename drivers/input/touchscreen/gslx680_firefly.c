...
static int gsl_reboot_notifier_handler(struct notifier_block *nb,
...
static bool tp_status = false;
static unsigned char demoBuffer[16];


void gsl_ts_irq_disable(struct gsl_ts *ts)
{
...
void gsl_ts_irq_enable(struct gsl_ts *ts)
{
...
#ifdef GSLX680_COMPATIBLE
	if (0x36 == chip_type) {
		if (global_fw == 1){		// for 10.1 lvds
            ptr_fw = GSL3680B_ORIGIN_FW;
            source_len = ARRAY_SIZE(GSL3680B_ORIGIN_FW);
		}else if(global_fw == 0){	// for face lvds
			ptr_fw = GSL3680B_FW;
			source_len = ARRAY_SIZE(GSL3680B_FW);
		}else if(global_fw == 2){	// for 10.1 mipi (3692 IC)
			ptr_fw = GSL3692_FW;
			source_len = ARRAY_SIZE(GSL3692_FW);
		}else{						// for edp & mipi
			ptr_fw = GSLX680_FW;
...
static void startup_chip(struct i2c_client *client)
{
	...
	if (is_noid_version){
		if (global_fw == 1)
			gsl_DataInit(gsl_config_data_id_origin_3680B);
		else if(global_fw == 0)
			gsl_DataInit(gsl_config_data_id_3680B);
		else if(global_fw == 2)
			gsl_DataInit(gsl_config_data_id_3692);
		else
...
static int gsl_config_read_proc(struct seq_file *m, void *v)
{
	...
	if ('v' == gsl_read[0] && 's' == gsl_read[1]) {
		...
		if ('i' == gsl_read[3]) {
			...
			i2c_smbus_write_i2c_block_data(gsl_client, 0xf0, 4,
						       &gsl_data_proc[4]);
			if (gsl_data_proc[0] < 0x80)
				i2c_smbus_read_i2c_block_data(gsl_client,
							      gsl_data_proc[0],
							      4, temp_data);
			i2c_smbus_read_i2c_block_data(gsl_client,
						      gsl_data_proc[0], 4,
...
static void firefly_ts_suspend(void)
{
	int i;
	//disable_irq_nosync(gts->irq);
	...
	#ifdef REPORT_DATA_ANDROID_4_0
	for (i = 1; i <= MAX_CONTACTS; i++) {
		input_mt_slot(gts->input, i);
		input_report_abs(gts->input, ABS_MT_TRACKING_ID, -1);
		input_mt_report_slot_state(gts->input, MT_TOOL_FINGER, false);
	}
	#else
	input_mt_sync(gts->input);
	#endif
	input_sync(gts->input);
	msleep(10);
	//report_data(gts, 1, 1, 10, 1);
	input_sync(gts->input);
#endif
}
...
static void firefly_ts_resume(void)
{
	int i;
	gslX680_shutdown_high();
	msleep(20);
...
int tp_ctrl_open(struct inode *inode, struct file *filp)
{
	//printk("wdt_open \r\n");
    return 0;
}

int tp_ctrl_release(struct inode *inode, struct file *filp)
{  
	return 0;
}

ssize_t tp_ctrl_read(struct file *filp, char __user *buf, size_t count,loff_t *f_pos)
{
	char kbuf[5] = {0};
	int ret, wrinten;
...
ssize_t tp_ctrl_write(struct file *filp, const char __user *buf, size_t count,loff_t *f_pos)
{
	/* 把数据复制到内核空间 */
	uint8_t len = (int)count;
	if(len>2)
	{
		printk("tp_ctrl:inviald value too long\r\n");
		return count;
	}
	...
	switch(demoBuffer[0])
	{
		case '0':	if(true == tp_status)
					{
						firefly_ts_suspend();
						tp_status = false;
					}
					break;
		case '1':	if(false == tp_status)
					{
						firefly_ts_resume();
						tp_status = true;
					}
					break;//2.56
		default:printk("tp_ctrl:inviald value \r\n");break;
	}
	//printk("wdt_weite %c\r\n", demoBuffer[0]);
	return count;
}

struct file_operations tp_ctrl_fops = {
	.owner = THIS_MODULE,
	.read =	tp_ctrl_read,
	.write = tp_ctrl_write,
	.open = tp_ctrl_open,
	.release = tp_ctrl_release,
};
			...
			const struct i2c_device_id *id)
{
	...
	ts->reboot_notifier.notifier_call = gsl_reboot_notifier_handler;
	ret = devm_register_reboot_notifier(&ts->client->dev,&ts->reboot_notifier);
	if (ret){
	...
	if (!gsl_config_proc) {
		print_info("create_proc %s failed\n", GSL_CONFIG_PROC_FILE);
	}
	gsl_proc_flag = 0;
#endif

	major = register_chrdev(0, "tp_ctrl", &tp_ctrl_fops);
    if (major < 0)
    {
	...
	// 创建类
	cls = class_create(THIS_MODULE, "tp_ctrl");
	// 创建设备节点
	dev = device_create(cls, NULL, MKDEV(major, 0), NULL, "tp_ctrl");
	tp_status = true;
	INIT_WORK(&firefly_tp_work, work_handler);
	schedule_work(&firefly_tp_work);
	printk("====== gslx3680 init success =====\r\n");
	return 0;
...
static int gsl_reboot_notifier_handler(struct notifier_block *nb,unsigned long action, void *cmd)
{
       gsl_ts_irq_disable(gts);
       gslX680_shutdown_low();
       return NOTIFY_OK;
