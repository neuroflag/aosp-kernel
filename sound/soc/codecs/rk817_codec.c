...
#define RK817_ADC_OVER_ADVALUE		1024
#define RK817_ADC_DRIFT_ADVALUE		24	
...
/*
* MICIN_PGA_GAIN L/R  setting
* -18db~-27db,3db/step,for example:
* 0: -18dB
* 0x66: 0dB
* 0xff: 27dB
 */
#define MICIN_PGA_GAIN (0x99)
...
extern int register_firefly_rk628_notifier(struct notifier_block*);
extern int unregister_firefly_rk628_notifier(struct notifier_block*);
//bool mic_in_line2_rk628d = false;
struct rk817_codec_priv* rk817_rk628d = NULL;
static int firefly_rk628d_event_trigger(struct notifier_block *this , unsigned long event , void*ptr)
{
    //printk("[zyk debug] rk628d notifier is triggered\n");

	if(event == 1 && rk817_rk628d->capture_path != MIC_OFF && rk817_rk628d != NULL){
			//mic_in_line2_rk628d = false;
			//rk817_codec_power_up(component, RK817_CODEC_CAPTURE);
			snd_soc_component_write(rk817_rk628d->component,
										RK817_CODEC_AMIC_CFG0,
										0x80);
			snd_soc_component_write(rk817_rk628d->component,
										RK817_CODEC_DMIC_PGA_GAIN,
										rk817_rk628d->micin_pga_gain);
			//printk("[zyk debug] adjust pga gain\n");
	}
    return 0;
}

static struct notifier_block firefly_rk628d_notifier =
{
        .notifier_call = firefly_rk628d_event_trigger,
};
				...
				int gpio, int level)
{
	if ((gpio & CODEC_SET_SPK) &&
	    gpio_is_valid(rk817->spk_gpio)){
		gpio_set_value_cansleep(rk817->spk_gpio, level);
		DBG("%s set spk clt %d\n", __func__, level);
	}
	if ((gpio & CODEC_SET_SPK) &&
	    !IS_ERR_OR_NULL(rk817->spk_ctl_gpio)) {
	...
	if ((gpio & CODEC_SET_HP) &&
	    !IS_ERR_OR_NULL(rk817->hp_ctl_gpio)) {
...
static int rk817_digital_mute(struct snd_soc_dai *dai, int mute)
{
	...
	if (mute) {
		...
		case SPK_HP:
		case RING_SPK_HP:
			if ( hp_enable_global && rk817->chan != NULL ){
				rk817_codec_ctl_gpio(rk817, CODEC_SET_SPK, 0);
				rk817_codec_ctl_gpio(rk817, CODEC_SET_HP, 1);
			}else if(rk817->chan != NULL){
				rk817_codec_ctl_gpio(rk817, CODEC_SET_SPK, 1);
				rk817_codec_ctl_gpio(rk817, CODEC_SET_HP, 0);
			}
			else{
...
*/
static int rk817_jack_init(struct rk817_codec_priv *rk817)
{
	int ret = 0;
	/*
        snd_soc_card_jack_new(rk817->component->card, "Headphone Jack", SND_JACK_HEADPHONE,
                        &rk817->hp_jack, NULL, 0);
        snd_soc_jack_notifier_register(&rk817->hp_jack, &rk817_hp_jack_nb);
        */
	rk817->edev = devm_extcon_dev_allocate(rk817->component->dev, headset_cable);
	if (IS_ERR(rk817->edev)) {
		dev_err(rk817->component->dev, "failed to allocate extcon device\n");
		ret = -ENOMEM;
		return ret;
	}
	ret = devm_extcon_dev_register(rk817->component->dev, rk817->edev);
	if (ret < 0) {
		dev_err(rk817->component->dev, "extcon_dev_register() failed: %d\n", ret);
		return ret;
	}
	return ret;
}
...
static void rk817_hp_adc_poll(struct work_struct *work)
{
	...
	{
		if (result < rk817->hp_det_adc_value + rk817->hp_adc_drift_scope && 
	                  result > rk817->hp_det_adc_value - rk817->hp_adc_drift_scope)
		{
			if (!rk817->hp_insert){
				...
				rk817_codec_ctl_gpio(rk817, CODEC_SET_SPK, 0);
				rk817_codec_ctl_gpio(rk817, CODEC_SET_HP, 1);
				extcon_set_state_sync(rk817->edev, EXTCON_JACK_HEADPHONE,true);
				//snd_soc_jack_report(&rk817->hp_jack, 1, SND_JACK_HEADPHONE);
			}
		}else{
			if (rk817->hp_insert){
				...
				rk817_codec_ctl_gpio(rk817, CODEC_SET_SPK, 1);
				rk817_codec_ctl_gpio(rk817, CODEC_SET_HP, 0);
				extcon_set_state_sync(rk817->edev, EXTCON_JACK_HEADPHONE,false);
				//snd_soc_jack_report(&rk817->hp_jack, 0, SND_JACK_HEADPHONE);
			}
		}
...
static int rk817_probe(struct snd_soc_component *component)
{
	...
	ret = register_firefly_rk628_notifier(&firefly_rk628d_notifier);
	if(ret)
		printk("[zyk debug] %s: register firelfy rk628 notifier failed\n",__func__);

	rk817->hp_insert = false;
        if (rk817->chan) {
                ret=rk817_jack_init(rk817);
                if(ret<0){
                	return ret;
                }
...
static void rk817_remove(struct snd_soc_component *component)
{
	...
	unregister_firefly_rk628_notifier(&firefly_rk628d_notifier);
					 ...
					 struct rk817_codec_priv *rk817)
{
	...
	if (!node) {
        ...
        if (!of_property_read_u32(node, "hp-det-adc-value", &adc_value))
        {
            rk817->hp_det_adc_value = adc_value;
			 if (!of_property_read_u32(node, "hp-adc-drift-scope", &adc_value))
                   rk817->hp_adc_drift_scope = adc_value;
              else
                    rk817->hp_adc_drift_scope = RK817_ADC_DRIFT_ADVALUE;

                  printk("%s: rk809 hp adc effective value of detection %d~%d\n", __func__, rk817->hp_det_adc_value-rk817->hp_adc_drift_scope, rk817->hp_det_adc_value+rk817->hp_adc_drift_scope);
        }
        else
        {
            chan = NULL;
	...
	ret = of_property_read_u32(node, "micin-pga-gain",
                                  &rk817->micin_pga_gain);
	if (ret < 0) {
			DBG("%s() Can not read property micin-pga-gain\n",
				__func__);
			rk817->micin_pga_gain = MICIN_PGA_GAIN;
	}
...
static void rk817_platform_shutdown(struct platform_device *pdev)
{
	...
	if (rk817 && rk817->component)
		rk817_codec_power_down(rk817->component,RK817_CODEC_IGNORE);
}
