...
extern struct device *g_sfc_dev;
//static struct completion sfc_irq_complete;
extern struct completion sfc_irq_complete;
...
static int rksfc_irq_config(int mode, void *pfun)
{
	...
	if (mode)
		...
		free_irq(irq,  g_sfc_info.reg_base);
	return ret;
}
...
static int rksfc_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev,
		 ...
		 g_sfc_info.clk_rate);
	rksfc_irq_init();
	if(of_property_read_bool(pdev->dev.of_node, "rk3566-fspi-two-line")){
		printk("[lzf debug][%s - %d] get rk3566-property \n",__func__,__LINE__);		
		rk3566_fspi_flag = true;
    }else{
    	rk3566_fspi_flag = false;
    }
#ifdef CONFIG_ROCKCHIP_THUNDER_BOOT
