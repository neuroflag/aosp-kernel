...
static int phy_rtl8211fd_vx_led_fixup(struct phy_device *phydev)
{
    int value;
...
static int phy_rtl8211f_disable_eee_fixup(struct phy_device *phydev)
{
	printk("hcq :  %s in\n", __func__);
		     ...
		     struct stmmac_resources *res)
{
	...
	/* Set led status */
	ret = of_property_read_u32(priv->device->of_node, "led_status_value", &led_value);
	if(ret){
		dev_info(priv->device, "[%s-%d]led_status_value = 0x6940\n",__func__,__LINE__);
		led_value = 0x6940;
	}else{
		dev_info(priv->device, "[%s-%d]led_status_value = 0x%x\n",__func__,__LINE__,led_value);
	}
	...
	ret = phy_register_fixup_for_uid(RTL_8211FD_VX_PHY_ID, 0xffffffff, phy_rtl8211fd_vx_led_fixup);
       if (ret)
               pr_warn("Cannot register 8211fd_VX PHY board fixup.\n");
	...
	ret = phy_register_fixup_for_uid(RTL_8211F_PHY_ID, 0xffffffff, phy_rtl8211f_disable_eee_fixup);
	if (ret)
		pr_warn("Cannot register RTL_8211F_PHY_ID PHY board disable eee fixup.\n");
	ret = phy_register_fixup_for_uid(RTL_8211F_VD_CG_PHY_ID, 0xffffffff, phy_rtl8211f_disable_eee_fixup);
	if (ret)
		pr_warn("Cannot register RTL_8211F_VD_CG_PHY_ID PHY board disable eee fixup.\n");
